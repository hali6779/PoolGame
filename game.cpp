...
Memento* Game::saveMemento(){
    return new Memento(this->m_balls);
}

void Game::loadMemento(Memento* tempMemento){
    if (tempMemento != nullptr){
        std::vector<Ball*>* temp = tempMemento->getBalls();
        this->m_balls->clear();
        for(Ball* ball: *temp){
            this->m_balls->push_back(ball);
        }
        m_mouseEventFunctions.clear();
        CueBall* cueball = tempMemento->getCueball();
        if (cueball){
            this->m_balls->push_back(cueball);
            addMouseFunctions(cueball->getEvents());
        }
    }
}
